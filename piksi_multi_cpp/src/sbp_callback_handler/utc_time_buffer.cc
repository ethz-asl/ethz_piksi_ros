...
ros::Time UtcTimeBuffer::getTime(const uint32_t tow) {
  if (tow_to_utc_.has_value() && tow_to_utc_.value().first == tow) {
    return tow_to_utc_.value().second;
  } else {
    ROS_WARN(
...
void UtcTimeBuffer::callback(uint16_t sender_id, uint8_t len, uint8_t msg[]) {
  ...
  auto tow = sbp_msg->tow;
  auto stamp = libsbp_ros_msgs::convertUtcTimeToRosTime(*sbp_msg);
  tow_to_utc_ = std::make_optional(std::make_pair(tow, stamp));
}
